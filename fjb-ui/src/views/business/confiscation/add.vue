  ...
  <div class="biaodan">
    <el-form ref="form" :model="form" :rules="rules"  label-width="140px" class="form_input">
      <el-row >
        ...
        <el-col :span="12">
          <el-form-item label="没收单位:"  prop="deptName">
        ...
        <el-col :span="12">
          <el-form-item label="鉴定证书编号" prop="unitOrIndividual">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        rules: {
          ...
          holderIdcard: [
            ...
            { min: 8, max: 20, message: '8~20个数字或字母', trigger: 'blur'}
          ],
          unitOrIndividual: [
    ...
    methods: {
      ...
      //获取id数据
      getReceiptseizure(id){
        getReceiptseizure(id).then(response => {
          this.form = response.data
          console.log(response.data.receiptdate)
          ...
          this.receiptdate_yuan=response.data.receiptdate;
          this.form.imgList='';
        });
      },
      // 取消按钮
      ...
      handleRemove(file) {
        ...
        let num=0;
        this.$refs['dynamic'].uploadFiles.map(item=>{
          if(item.uid==file.uid){
            this.$refs['dynamic'].uploadFiles.splice(num,1);
          }
          num++;
        })
      },
      ...
      //提交表单
      submitForm: function(){
        console.log(this.form.receiptdate)
        this.$refs["form"].validate(valid => {
          if (valid) {
            const id = this.$route.params && this.$route.params.id;
            if(id!=0){
              if(this.serialNumber_yuan==this.form.serialNumber&&this.receiptdate_yuan==this.form.receiptdate){
                this.msgError('收缴日期或者冠字号码必须有一种和模板不一样!')
                return;
              }
            }
            let arr=this.$refs['dynamic'].uploadFiles
            if(arr.length>=this.zhang) {
              ...
              if(count<this.zhang){
                ...
                axios.post(process.env.VUE_APP_BASE_API+"/business/receiptseizure/add", this.upload.formDate, config).then(res => {
