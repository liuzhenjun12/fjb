...
public class SysDeptServiceImpl implements ISysDeptService
{
    ...
    public String checkDeptCodeUnique(SysDept dept) {
        Long deptId = StringUtils.isNull(dept.getDeptId()) ? -1L : dept.getDeptId();
        SysDept info = deptMapper.checkDeptCodeUnique(dept.getJigouCode());
        if (StringUtils.isNotNull(info) && info.getDeptId().longValue() != deptId.longValue())
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    public List<Cascader> buildCascadeselect(List<SysDept> depts) {
        List<SysDept> deptTrees = buildDeptTree(depts);
        return deptTrees.stream().map(Cascader::new).collect(Collectors.toList());
    }
